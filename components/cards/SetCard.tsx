...
export function SetCard({ set, latestPrice, prevPrice }: Props) {
  ...
  return (
    <Link
      href={`/cards/${set.id}`}
      className="card flex flex-col gap-3 hover:-translate-y-0.5 transition"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-xs text-ink-muted">{set.series}</div>
      ...
      <div className="text-xs text-ink-muted flex flex-wrap gap-x-3 gap-y-1">
        <span>발매 {formatDate(set.releaseDate)}</span>
